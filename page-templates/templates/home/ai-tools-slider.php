  ...
  <section class="max-w-[1440px] mx-auto">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
      <div class="flex items-center gap-2">
        ...
        <h1 class="text-[22px] sm:text-[40px] font-semibold">AI <span class="text-[var(--primary)]">Tools</span></h1>
      </div>
      <div class="flex justify-end">
        <a href="/ai-tools" class="bg-[var(--primary)] w-fit h-fit text-white py-2 px-3 sm:px-5 rounded-md">View All</a>
      </div>
    </div>
          ...
          <div class="swiper-wrapper">
            ...
            if ($ai_tools->have_posts()):
              while ($ai_tools->have_posts()):
                $ai_tools->the_post();
                ?>
            <div class="swiper-slide tool-slide" data-tags="<?php $tags = get_the_terms(get_the_ID(), 'ai-tool-tag');
                echo esc_attr($tags && !is_wp_error($tags) ? implode(',', array_map(function ($tag) {
                  return $tag->name;
                }, $tags)) : ''); ?>" data-img="<?php echo esc_url(has_post_thumbnail() ? get_the_post_thumbnail_url(get_the_ID(), 'medium') : 'https://digitalmarketingsupermarket.com/wp-content/uploads/2025/05/Saly-1.png'); ?>" data-title="<?php echo esc_attr(get_the_title()); ?>" data-excerpt="<?php echo esc_attr(wp_trim_words(get_the_excerpt(), 20)); ?>" data-link="<?php echo esc_url(get_permalink()); ?>">
